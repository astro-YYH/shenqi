 ...
 */
__device__ double wk_cs_device(DensityKernel * kernel, double q) {
    if(q < 1.0) {
...
__device__ double dwk_cs_device(DensityKernel * kernel, double q) {
    if(q < 1.0) {
...
__device__ static double wk_qus_device(DensityKernel * kernel, double q) {
    if(q < 0.5) {
...
__device__ static double dwk_qus_device(DensityKernel * kernel, double q) {
    if(q < 0.5) {
...
__device__ static double wk_qs_device(DensityKernel * kernel, double q) {
    if(q < 1.0) {
...
__device__ static double dwk_qs_device(DensityKernel * kernel, double q) {
    if(q < 1.0) {
...
__constant__ struct {
    ...
    { "CubicSpline", wk_cs_device, dwk_cs_device, 2.,
        {2 / 3., 10 / (7 * M_PI), 1 / M_PI} },
    { "QuinticSpline", wk_qs_device, dwk_qs_device, 3.,
        {1 / 120., 7 / (478 * M_PI), 1 / (120 * M_PI)} },
    { "QuarticSpline", wk_qus_device, dwk_qus_device, 2.5,
...
__device__ double
density_kernel_volume_device(DensityKernel * kernel)
{
    return NORM_COEFF * pow(kernel->H, NUMDIMS);
}

__device__ static void
density_kernel_init_with_type_device(DensityKernel * kernel, int type, double H)
{
    kernel->H = H;
    kernel->HH = H * H;
    kernel->Hinv = 1. / H;
...
__device__ void
density_kernel_init_device(DensityKernel * kernel, double H, enum DensityKernelType type)
{
    int t = -1;
    ...
    density_kernel_init_with_type_device(kernel, t, H);
}
